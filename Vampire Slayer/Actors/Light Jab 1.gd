...
func update():
	if return_to_idle:
		...
		return
	if player.animation_player.current_animation != "Light Jab 1":
		player.animation_player.play("Light Jab 1")
		_charging = false
		_charge_counter = 0
	...
	if Input.is_action_just_pressed("attack") and _charge_counter < 20 and attack_buffer_ready:
		_charging = false
		_charge_counter = 0
		state.update_state("Light Jab 2")
		pass
	pass
